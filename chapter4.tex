...
\begin{rem}
 ...
 \begin{enumerate}[label=\emph{\alph*}),leftmargin=*]
  ...
  \item \label{enum: commutator contravariant}
   If $T \subseteq S$ then $S' \subseteq T'$.
  \item \label{enum: commutator adjoint}
   We have $S \subseteq T'$ if and only if $T \subseteq S'$, since both are equivalent to
   \[
    st = ts \text{ for every } t \in T, s \in S.
   \]
  \item
   We have $S' \subseteq S'$ and thus $S \subseteq S''$ by \ref{enum: commutator adjoint}.
  \item
   Since $S \subseteq S''$ we also have $S''' = (S'')' \subseteq S'$ by \ref{enum: commutator contravariant}. By \ref{enum: commutator adjoint} we also have $S' \subseteq (S')'' = S'''$. So together we have $S' = S'''$.
 \end{enumerate}
\end{rem}
