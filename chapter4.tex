...
\begin{defi}
 ...
 \]
 be the corresponding algebra homomorphism. Then the character $\chi_V$ of $V$ is defined as
 \[
...
\begin{proof}
 ...
 \begin{enumerate}[label=\emph{\alph*})]
  ...
  \item
   ...
   \end{align*}
  \item
   Let $M \in \Mat_s(k)$ be the representing matrix of $\rho_V(g)$ with respect to the basis $v_1$, \dots, $v_s$ and $A^* \in \Mat_s(k)$ be the representing matrix of $\rho_{V^*}(g)$ with respect to the basis $v_1^*$, \dots, $v_s^*$. Then $A^{-1}$ is the representing matrix of $\rho_V(g^{-1}) = \rho_V(g)^{-1}$ with respect to the basis $v_1$, \dots, $v_s$. We also know from the tutorial problems that $A^* = \left(A^{-1}\right)^T$. Daher ist
   \begin{align*}
    \chi_{V^*}(g)
    &= \tr \rho_{V^*}(g)
    = \tr A^*
