...
\begin{defi}
 ...
 \]
 is the commutant or centralizer of $S$ in $\End_\Z(M)$. In the same way we define for a subseteq $S \subseteq \End_R(M)$
 \[
  S'_R \coloneqq \{\varphi \in \End_R \mid \varphi s = s \varphi \text{ for all } s \in S\}
 \]
 as the commutant or centralizer of $S$ in $\End_R(M)$.
\end{defi}
