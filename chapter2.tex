...
We also set
\[
 P_l(t) := \sum_{k \geq 0} p^{(n)}_{k+l} t^k
\]
for all $l \geq 0$. (Notice that $P = P_0$.)
...
\begin{prop}
 ...
 \begin{enumerate}[1)]
  ...
  \item
   $P(t) = \sum_{i=1}^n \frac{1}{1 - X_i t}$. More generally $P_l(t) = \sum_{i=1}^n \frac{X_i^l}{1 - X_i t}$ for all $l \geq 0$.
 \end{enumerate}
\end{prop}
\begin{proof}
 \begin{enumerate}[1)]
...
\begin{cor}
 ...
 \[
  h_s - e_1 h_{s-1} + e_2 h_{s-2} + \ldots + (-1)^{s-1} e^{s-1} h_1 + (-1)^s e_s = 0
 \]
 as well as
 \[
  e_s - h_1 e_{s-1} + h_2 e_{s-2} + \ldots + (-1)^{s-1} h^{s-1} e_1 + (-1)^s h_s = 0.
 \]
\end{cor}
\begin{proof}
