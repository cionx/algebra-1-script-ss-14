...
\begin{thrm}[Fundamental Theorem of symmetric functions]
 The symmetric polynomials $e^{(n)}_1, \ldots, e^{(n)}_n$ generate the $k$-algebra of symmetric functions $k[X_1, \ldots, X_n]^{S_n}$. Moreover they are algebraically independent. Hence
 \[
  k[X_1, \ldots, X_n]^{S_n} \cong k\left[e^{(n)}_1, \ldots, e^{(n)}_n\right] \text{ via } X_i \mapsto e^{(n)}_i
 \]
 as $k$-algebras.
\end{thrm}
...
\begin{proof}[Proof of the Fundamental Theorem]
 ...
 \begin{proof}[Proof of claim]
  ...
  \]
  (Here we use that $k[X_1, \ldots, X_n]$ is an unique factorization domain.) We have $\deg h < \deg f = d$, since $\deg g \leq \deg f$. By induction hypothesis (of the induction on $d$) we can write $h$ as a polynomial in $e^{(n)}_1, \ldots, e^{(n)}_n$ and therefore also $f = h e^{(n)}_n$.
