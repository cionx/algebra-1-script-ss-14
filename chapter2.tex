...
\begin{expls}\leavevmode
 \begin{enumerate}[a)]
  ...
  \[
   A_d :=
   \begin{cases}
    \vspan_k(\{X_1^{\alpha_1} \cdots X_n^{\alpha_n} \mid \sum_{i=1}^n a_i = d\}) & \text{if } d \geq 0, \\
    0                                                                            & \text{otherwise}.
   \end{cases}
  \]
...
\begin{proof}[Proof of the Fundamental Theorem (lecture)]
 ...
 \begin{proof}[Proof of claim]
  ...
  \[
   q \colon k[X_1, \ldots, X_n] \to k[X_1, \ldots, X_n]/(X_n) \cong k[X_1, \ldots, X_{n-1}]
  \]
...
\begin{defi}
 By $\mc{P}(V)$ we denote set of polynomial functions $V \to k$, i.e. $f \in \mc{P}(V)$ if and only if $f \colon V \to k$ and there is some $p \in k[X_1, \ldots, X_n]$ such that
 \[
...
\end{align*}
for some $p' \in k[X_1, \ldots, X_n]$. So if $f \colon V \to k$ is a polynomial in $(v_1, \ldots, v_n)$ then also in $(w_1, \ldots, w_n)$.
...
\begin{proof}
 ...
 \[
  \Phi \colon k[X_1, \ldots, X_n] \to \mc{P}(V), p \mapsto \Phi(p)
 \]
 where
 \[
...
\begin{cor}
 The map $\Phi \colon k[X_1, \ldots, X_n] \to \mc{P}(V), X_j \mapsto \varphi_j$ is injective.
\end{cor}


Together with the exercise sheet we find that
\[
 \Phi \colon k[X_1, \ldots, X_n] \to \mc{P}(V), X_j \to \varphi_j
\]
...
\begin{rem}
 ...
 \[
  \Phi \colon k[X_1, \ldots, X_n] \to \mc{P}(k^n), X_i \mapsto \varphi_i,
 \]
...
\begin{defi}
 Let $V$ and $W$ be finite dimensional $k$-vector spaces. A map $f \colon W \to V$ is called a polynomial map if, given a basis $v_1, \ldots, v_n$ of $V$, the coordinate functions of $f$ are polynomial, i.e. there exist $f_1, \ldots, f_n \in \mc{P}(W)$ with
 \[
...
\begin{expl}
 ...
 \[
  f \colon W \to W^{\otimes r}, w \mapsto w \otimes \ldots \otimes w.
 \]
...
\]
 ...
 \[
  \Psi \colon \mc{P}(W)^n \to \Hom_{\cAlg{k}}(\mc{P}(V),\mc{P}(W)), (f_1, \ldots, f_n) \mapsto (\varphi_j \mapsto f_j)
 \]
 ...
 \[
  \Phi \colon \mc{P}(W)^n \to \Pol_k(W,V), (f_1, \ldots, f_n) \mapsto \left(w \mapsto \sum_{i=1}^n f_i(w)v_i\right)
 \]
 is bijective.
 
 We notice that for a polynomial map $f \colon W \to V$ and $f_1, \ldots, f_n \in \mc{P}(W)$ with
 \[
  f(w) = \sum_{i=1}^n f_i(w) v_i \text{ for all } w \in W
 \]
...
\begin{expls}
 \begin{enumerate}\leavevmode
  ...
  \[
   \beta \colon W \to W^{\otimes d}, w \mapsto w \otimes \ldots \otimes w
  \]
