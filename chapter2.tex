...
\begin{rem}[Newton’s identities]
 ...
 \[
  p^{(n)}_r - e^{(n)}_1 p^{(n)}_{r-1} + \dotsb + (-1)^{r-1} e^{(n)}_{r-1} p^{(n)}_1 + (-1)^r r e^{(n)}_r = 0.
 \]
 for all $r \geq 0$. For $m = n$ this gives us the previous corollary.
\end{rem}
\begin{proof}
...
\begin{cor}
 ...
 \[
  r h^{(n)}_r = p^{(n)}_1 h^{(n)}_{r-1} + p^{(n)}_2 h^{(n)}_{r-2} + \dotsb + p^{(n)}_{r-1} h^{(n)}_1 + p^{(n)}_r.
 \]
\end{cor}
\begin{proof}
...
\[
 \mc{P}_L(W_L) \cong L[X_1, \dotsc, X_n]
\]
depends on choosing an $L$-basis of $W_L$ we can not expect this isomorphism $\mc{P}_k(W)_L \cong \mc{P}(W_L)$ to be independent of such choice. We will come back to this later.
...
\]
 ...
 \tikzsetnextfilename{fully_faithful_functor}
 \begin{figure}\centering
