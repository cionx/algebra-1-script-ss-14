...
\begin{defi}
 Let $W$ be a finite-dimensional $k$-vector space and \mbox{$S \subseteq \mc{P}(W)$}. Then
 \[
  \mc{V}(S) \coloneqq \{x \in W \mid f(x) = 0 \text{ for all } f \in S\}.
 \]
 is the zero set or varishing set associated with $S$, or affine algebraic varity corresponding to S.
\end{defi}
...
\begin{lem}
 Let $W$ be a finite dimensional $k$-vector space and $S \subseteq \mc{P}(W)$. For $I \coloneqq (S)_{\mc{P}(W)}$ we have
 \[
  \mc{V}(S) = \mc{V}(I).
 \]
\end{lem}
