...
\begin{thrm}(Hilbert)
 ...
 \]
 where $L_i \subseteq \mc{P}(W)$ is an irreducible representation of $G$ for all $i \in I$. Then $\mc{P}(W)^G$ is finitely generated.
\end{thrm}


\begin{lem}
 Let $A = \bigoplus_{d \geq 0} A_d$ be a graded $k$-algebra which is commutative. Assume that the ideal $A^+ = \bigoplus_{d > 0} A_d$ is finitely generated over $A$. Then $A$ is finitely generated as an $A_0$-algebra by homogeneous elements. (By this we mean that there are finitely many homogeneous elements $a_1, \dotsc, a_n \in A^+$ such that $A = A_0[a_1, \dotsc, a_n]$.)
\end{lem}
\begin{proof}
...
\begin{thrm}
 Let $G$ be a group, $W$ a representation of $G$ over $k$ such that $W = \bigoplus_{i \in I} L_i$ where $L_i \subseteq W$ is an irreducible representation for all $i \in I$. Then $\mc{P}(W)^G$ is finitely generated as a $k$-algebra.
\end{thrm}
