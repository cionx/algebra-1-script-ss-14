...
\begin{proof}
 ...
 \[
  \varphi \colon k[X_{11}, \ldots, X_{nn}] \to k[t]
 \] 
...
\begin{proof}
 ...
 \[
  \varphi \colon k[X_{11}, \ldots, X_{nn}] \to k[t_1, \ldots, t_n]
 \]
 is the algebra homomorphism defined by
 \[
  \varphi(X_{ij}) =
 ...
 \[
 \Phi \colon k[t_1, \ldots, t_n] \to \mc{P}(k^n).
 \]
...
\begin{expls}\leavevmode
 \begin{enumerate}[a)]
  ...
  \item
  Let $W$ be a finite $k$-vector space and $U \subsetneq W$ a vector subspace. Then $U$ is not Zariski dense in $W$. To see this let $w_1, \ldots, w_n, w_{n+1}, \ldots, w_m$ be a basis of $W$ such that $w_1, \ldots, w_n$ is a basis of $U$. Since $U \neq W$ we have $n < m$. We define $\pi \in \mc{P}(W)$ as the projection
  \[
...
\end{prop}
\begin{proof}
 Let $f \in \mc{P}(\Mat_n(k))$ with $f_{|\Diag_n(k)} = 0$ and $A \in \Mat_n(k)$. Since $k$ is algebraically closed we find $S \in \GL_n(k)$ such that $SAS^{-1}$ is in Jordan normal form with eigenvalues $b_1, \ldots, b_n$ (not necessarily pairwise distinct).
 
 Let $a_1, \ldots, a_n \in k^\times$ be pairwise different (this is possible since $k$ is infinite). We define
 \[
