...
For this section we fix an infinte field $k$.
...
\[
 \chi_A(t) = t^n - s_{n-1}(A) t^{n-1} + s_{n-2}(A) t^{n-2} + \ldots + (-1)^n s_0
\]
where $s_0, \ldots, s_{n-1} \in \mc{P}(\Mat_n(k))$. In particular $s_0 = \det$ and $s_{n-1} = \tr$.
...
Therefore
\[
 s_i(\diag(d_1, \ldots, d_n)) = e_{n-i}(\diag(d_1, \ldots, d_n))
\]
for all $0 \leq i \leq n-1$ and every diagonal matrix $\diag(d_1, \ldots, d_n)$.
