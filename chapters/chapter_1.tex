...
\begin{example}
  Let the symmetric group $S_n$ acts on $k^n$ so that
  \[
    \sigma.e_i = e_{\sigma(i)}
  \]
  for all $\sigma \in S_n$, $i = 1, \dotsc, n$, where $e_1, \dotsc, e_n$ denotes the standard basis of $k^n$.
  \begin{enumerate}
    \item
      The linear subspaces $U_1, U_2 \subseteq k^n$ given by
      \begin{gather*}
                  U_1
        \defined  \vspan\left( \sum_{i=1}^n e_i \right) \,,
        \qquad
                  U_2
        \defined  \left\{
    ...
    \item
      The subrepresentation $U_1$ is the one-dimensional trivial representation, and both $U_1$ and $U_2$ are irreducible.
    \item
      If $n = 2$ and $\kchar k = 2$, then $U_1 = U_2$, and otherwise $U_1 \ncong U_2$.
    \item
