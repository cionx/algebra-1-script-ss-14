...
Notice that for every $x \in X$ we have $\supp(\chi_x) = \{x\}$ and thus $\chi_x \in kX$.
...
\begin{expls}
  \begin{enumerate}[label=\emph{\alph*)},leftmargin=*]
    ...
    \item
      ...
      So $\tau'$ it is the unique action making the diagram in figure \ref{fig: action on product and tensor} commute for all $g \in G$.
      ...
      \end{figure}
      If $v_1$, \dots, $v_n$ is a basis of $V$ and $w_1$, \dots, $w_m$ a basis of $W$ we can write the action of $g \in G$ on $V$, resp.\ $W$, as a matrix $A$, resp.\ $B$.
