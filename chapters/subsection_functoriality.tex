...
\begin{fluff}
  ...
  \[
              f_L
    \coloneqq {\id_L} \tensor f
    \colon    V_L
    \to       W_L
  \]
  is already $L$-linear:
  For all $\lambda \in L$ and simple tensors $l \tensor v \in V_L$ with $l \in L$, $v \in V$ we have that
  \[
      f_L( \lambda \cdot (l \tensor v) )
    = f_L( (\lambda l) \tensor v )
    = (\lambda l) \tensor f(v)
    = \lambda \cdot (l \tensor f(v))
    = \lambda f_L(l \tensor v) \,,
  \]
  ...
  \[
      g_L \circ f_L
    = ({\id_L} \tensor g) \circ ({\id_L} \tensor f)
    = (\id_L \circ \id_L) \tensor (g \circ f)
    = {\id_L} \tensor (g \circ f)
    = (g \circ f)_L \,.
  \]
...
\begin{remark}
  ...
  \[
              B_L
    \defined  1 \tensor B
    =         \{1 \tensor b \suchthat b \in B\}
  \]
  of $V_L$ and the basis $C_L \defined 1 \tensor C$ of $W_L$ we have the equality of representing matrices
  \[
      [f]^B_C
    = [f_L]^{B_L}_{C_L} \,.
  \]
