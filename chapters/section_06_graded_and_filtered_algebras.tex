...
\begin{remark}
  ...
  \begin{enumerate}
    \item
      In place of the natural numbers $\Natural$ one can also define gradings over $M$:
      
      For a monoid $M = (M,\cdot)$ an $M$-grading of a $k$-algebra $A$ is a decomposition $A = \bigoplus_{m \in M} A_m$ into $k$-linear subspaces $A_m \subseteq A$ such that $A_m A_{m'} \subseteq A_{mm'}$ for all $m, m' \in M$.
      An $M$-graded $k$-algebra is a $k$-algebra $A$ together with an $M$-grading $A = \bigoplus_{m \in M} A_m$.
    ...
    \item
      ...
      If the monoid $M$ is right cancellative (i.e.\ it follows for all $m_1, m_2, m \in M$ from $m_1 m = m_2 m$ that $m_1 = m_2$) then it still follows from the calculations of part~b) of Remark~\ref{remark: connection between graded algebras and rings} that $1 \in R_1$, where $1$ denotes the neutral element of $M$.
      By using the identity $x = x \cdot 1$ instead of $x = 1 \cdot x$ in the calculation it follows that this also holds if $M$ is right cancellative.
      
      This holds in particular if $M$ is a group or a submonoid of a group.
      
      In then follows that $R_1$ is a subring of $R$ and that for every $m \in M$ the homogeneous part $R_m$ inherits the structure of an $R_0$-$R_0$-bimodule from the multiplication of $R$.
    \item
      Suppose that $N \subseteq M$ is a submonoid, i.e.\ we have that $1_M \in N$ and $n_1 n_2 \in N$ for all $n_1, n_2 \in N$.
      ...
      As a special case of this construction every $\Natural$-grading of an $k$-algebra $A$ (resp.\ ring $R$) can be regarded as an $\Integer$-grading with $A_d = 0$ (resp.\ $R_d = 0$) for all $d < 0$.
      Indeed, the definition of a grading given in the lecture did not use a $\Natural$-grading as we have in done in Definition~\ref{definition: gradings and graded algebras} but a $\Integer$-grading.
      But all the examples and applications of graded $k$-algebras presented in this lecture were actually only using a $\Natural$-gradings, so we adjusted the definition accordingly.
  \end{enumerate}
\end{remark}
