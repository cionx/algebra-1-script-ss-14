...
\begin{corollary}
  ...
  \begin{enumerate}
    \item
      We have that $A \cong \Mat_n(D)$ for some $n \geq 1$ and skew field $D$ over $k$.
    \item
...
\begin{example}
  ...
  \begin{enumerate}
    \item
      The (first) Weyl algebra $\mc{A}_1$ from Example~\ref{example: weyl algebra} is simple but not semisimple.
    \item
      Let $V$ be an infinite-dimensional $k$-vector space and let $I \idealeq \End_k(V)$ be a maximal two-sided ideal (which exists by Zorn’s lemma).
