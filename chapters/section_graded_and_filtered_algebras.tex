...
\begin{fluff}
  ...
  It also follows from Lemma~\ref{lemma: associated graded reflects no zero divisors} that $\mc{A}'$ has no zero divisors because $\gr_{F'}(\mc{A}')$ has no zero divisors.
\end{fluff}
...
\begin{fluff}
  ...
  \]
  and the monomials $\xi^n \del^m$ with $n, m \geq 0$ are a basis of $\mc{A}$.
  We also have that
  \[
  ...
  The associated graded algebra $\gr_F(\mc{A})$ is the commutative polynomial ring in the two free variables $\xi$ and $\del$.
  This shows that the filtration $F$ of $\mc{A}$ does not come from a filtration of $\mc{A}$, and it follows from Lemma~\ref{lemma: associated graded reflects no zero divisors} that $\mc{A}$ has no zero divisors.
\end{fluff}


\begin{remark}(Skew polynomial rings)
  We have seen above that the first Weyl algebra $\mc{A}$ can be thought about in two ways:
  \begin{itemize}
    \item
  ...
  \end{itemize}
  Yet another way to think about $\mc{A}$ is provided by the language of skew polynomial rings and Ore extensions:
  
  We may replace the polynomial ring $k[\xi] \subseteq \mc{A}$ by the polynomial ring $k[x]$ and rename the generator $\del$ of $\mc{A}$ to $y$.
  We then have that
  \[
    yx = xy + 1 \,,
  \]
  ...
  We can therefore think about the Weyl algebra $\mc{A}$ as resulting from $k[x]$ by adjoining a new variable $y$ with $y p = \del(p)$ for all $p \in k[x]$.
  ...
  For $R = k[x]$ and $\delta = \del$ we have seen above that $k[x][y,\del] \cong \mc{A}$.
  
  In addition to the $k$-derivation one can also consider a $k$-algebra homomorphism $\alpha \colon R \to R$:
  A map $\delta \colon R \to R$ is then a $\alpha$-derivation if
  \[
      \delta(rs)
  ...
  \]
  for every $r \in R$.
  This $k$-algebra is then denotey by $R[y;\alpha,\delta]$ is an \emph{Ore extension} of $R$.
  For $\alpha = \id_R$ we retrieve the above the nototion of a skew polynomial ring.
...
\begin{remark}
  Let $\kchar k = 0$.
  One can more generally consider for every $n \geq 0$ the $n$-th Weyl algebra $\mc{A}_n$, which can be defined in multiple ways:
  \begin{itemize}
    \item
      The $k$-algebra $\mc{A}_n$ can be defined as the $k$-algebra of linear differential operators of $k[x_1, \dotsc x_n]$, i.e.\ the $k$-subalgebra of $\End_k(k[x_1, \dotsc, x_n])$ generated by $\xi_1, \dotsc, \xi_n$, where $\xi_i$ is the multiplication with $x_i$, and the partial derivatives $\del_1, \dotsc, \del_n$.
    \item
      The $k$-algebra $\mc{A}_n$ can be described by the generators $X_1, \dotsc, X_n, D_1, \dotsc, D_n$ and relations
      \[
        \begingroup
        \arraycolsep = 12pt
        ...
        \begin{array}{ll}
          ...
          \\
            \text{$D_i X_j = X_j D_i$ for $i \neq j$},
          & \text{$D_i X_i = X_i D_i + 1$ for all $i$} \,.
        \end{array}
        \endgroup
      \]
    \item
      The $n$-th Weyl algebra $\mc{A}_n$ can be constructed from the first Weyl algebra $\mc{A}_1$ as the $n$-fold tensor product $\mc{A}_1 \otimes \dotsb \otimes \mc{A}_1$.
    \item
      By defining more generaly the first Weyl algebra $\mc{A}_1(R)$ for every $k$-algebra $R$, the $n$-th Weyl algebra $\mc{A}_n(R)$ can inductively be constructed as $\mc{A}_n(R) = \mc{A}_1(\mc{A}_{n-1}(R))$.
  \end{itemize}
\end{remark}
