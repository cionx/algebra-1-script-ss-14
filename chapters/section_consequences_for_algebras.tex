...
% 
% \begin{conventions}
%   We will write $\tensor_k \defines \tensor$.
% \end{conventions}
% 
% 
% \begin{fluff}
%   If $N$ is a $k$-vector space and $M$ is an $A$-module then $A \tensor M$ can be endowed with structure of an $A$-module via
%   \[
%       a \cdot (n \tensor m)
%     = n \tensor (am)
%   \]
%   for all $a \in A$ and simple tensors $n \tensor m \in N \tensor M$.
...
%     \item
%       When the $k$-vector space $\Hom_A(E,M) \tensor E$ is endowed with the structure of an $A$-module via
%       \[
%           a \cdot (f \tensor e)
%         = f \tensor (a e)
%       \]
%       for all $a \in A$ and simple tensors $f \tensor e \in \Hom_A(E,M) \tensor E$ then $\Phi$ is $A$-linear.
%     \item
...
%       This shows that $\Phi(f \tensor e) = f(e) \in M_E$ for every simple tensor $f \tensor e \in \Hom_A(E,M) \tensor E$, which shows that $\im(\Phi) \subseteq M_E$.
%       
...
%     \item
%       We have for every simple tensor $f \tensor e \in \Hom_A(E,M) \tensor E$ and $a \in A$ that
%       \[
%           \Phi(a (f \tensor e))
%         = \Phi(f \tensor (ae))
%         = f(ae)
...
%           Then $\Phi$ maps $f_i \tensor E$ bijectively onto the submodule $L_i \moduleleq M$ (because $f_i$ is a bijection).
%           This shows that $\Phi$ maps the $i$-th summand of
%           \[
%               \Hom_A(E, M) \tensor E
%             = \bigoplus_{i=1}^n k f_i \tensor E
%             = \bigoplus_{i=1}^n f_i \tensor E
%           \]
...
%   \[
%             \bigoplus_{i \in I} \Hom_A(E_i, M) \tensor E_i
%     \to     M \,,
%     \quad   f \tensor e
%     \mapsto f(e)
%   \]
...
\begin{corollary}[Artin--Wedderburn]
  ...
  \begin{enumerate}
    \item
      We have that $A \cong \Mat_{n_1}(k) \times \dotsm \times \Mat_{n_r}(k)$ as $k$-algebras with $r \geq 0$ an $n_1, \dotsc, n_r \geq 1$.
      The number $r$ is uniquely determined as the number of simple $A$-modules up to isomorphism, and if $V_1, \dotsc, V_r$ is a set of representatives for the isomorphism classes of simple $A$-modules then the numbers $n_1, \dotsc, n_r$ agree with the numbers $\dim V_1, \dotsc, \dim V_r$ up to permutation.
    \item
...
\begin{corollary}
  ...
  If $k$ is algebraically closed and $A$ is finite-dimensional and semisimple then $\dim_k \ringcenter(A)$ is the number of isomorphism classes of simple $A$-modules.
\end{corollary}
...
\begin{corollary}[Double Centralizer Theorem]
  ...
  \begin{enumerate}
    ...
    \item
      Each $(A' \tensor_k A)$-submodule $W_i$ is of the form $W_i \cong V'_i \tensor_{D_i} V_i$ for a simple $A$-module $V_i$, a simple $A'$-module $V'_i$ and a skew field $D_i$ with $D_i \cong \End_A(V_i)$ and $D_i^\op \cong \End_{A'}(V'_i)$.
