...
\begin{definition}
  Let $V$ be a finite-dimesional representation of a group $G$.
...
\begin{example}
  ...
  The \hyperref[theorem: fundamental theorem of symmetric functions]{Fundamental Theorem of Symmetric Functions} shows that $\mc{P}(k^n)^{S_n}$ is again a polynomial ring in $n$ variables and even gives an explicit set of generators $e_1, \dotsc, e_n$.
\end{example}
...
\begin{fluff}
  ...
  We will determine two more invariant rings in this section:
  The invariant ring of the action of $\SL_n(k)$ on $\Mat_n(k)$ by left multiplication and the invariant ring of the action of $\GL_n(k)$ on $\Mat_n(k)$ by conjugation.
  This content is mostly taken from \cite[\S 1.2, \S 2.3]{InvariantPrimer}.
...
\begin{proof}
  ...
  It follows from the fist Zariski density property that $g = 0$.
