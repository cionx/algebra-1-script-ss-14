...
\begin{lemma}
  ...
  \begin{enumerate}
    \item
      For $X \subseteq V$ the vanishing ideal $\mc{I}(X)$ is an ideal in $\mc{P}(V)$.
    \item
...
\begin{corollary}
  ...
  Let $f \colon \Mat_n(k) \to k$ be a polynomial function and let $\overline{f} \colon \Mat_n(L) \to L$ be the unique $L$-polynomial extension of $f$ (as described in Proposition~\ref{proposition: consistent choice of identification for polynomial functions}).
  \begin{enumerate}
    \item
...
\begin{proof}
  ...
  The map $f$ is $\GL_n(k)$-invariant if and only if $\restrict{\Phi}{\GL_n(k) \times \Mat_n(k)} = 0$ and the map $\overline{f}$ is $\GL_n(L)$-invariant if and only if $\restrict{\Phi}{\GL_n(L) \times \Mat_n(L)} = 0$.
  ...
  It therefore follows from $\restrict{\Phi}{\GL_n(k) \times \Mat_n(k)} = 0$ that $\restrict{\Phi}{\GL_n(L) \times \Mat_n(L)} = 0$ showing part~a).
  
  By replacing $\GL_n$ by $\SL_n$ in the above argumentation we find that part~b) holds.
\end{proof}
...
\begin{proposition}
  Let $\GL_n(k), \SL_n(k)$ act on $\Mat_n(k)$ by conjugation.
  Then
  \[
      \mc{P}(\Mat_n(k))^{\GL_n(k)}
    = \mc{P}(\Mat_n(k))^{\SL_n(k)} \,.
...
\begin{proof}
  ...
  If $A, B$ are $\GL_n(k)$-conjugated then they are already $\SL_n(k)$-conjugated because for $S \in \GL_n(k)$ with $S A S^{-1} = B$ there exists some $\omega \in k$ with $\omega^n = 1/\det(S)$, and for $T \defined S/\omega \in \SL_n(k)$ we then have that $T A T^{-1}$.
  It follows that the actions of $\GL_n(k)$ and $\SL_n(k)$ on $\Mat_n(k)$ have the same orbits, which proves the claim in this case.
  ...
  Then $\overline{f}$ is already $\GL_n(L)$-invariant by the above argumentation.
  It then follows that the restriction of $f$ to $\Mat_n(k)$, which is precisely $f$, is $\GL_n(k)$-invariant.
\end{proof}
