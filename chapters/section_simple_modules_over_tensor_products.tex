...
%   Note that when $\Phi \colon A \to \End_k(M)$ and $\Psi \colon B \to \End_k(N)$ are the $k$-algebra homomorphisms corresponding to the above module structures, then the $k$-algebra homomorphism $A \otimes B \to \End_k(M \otimes B)$ corresponding to the $(A \otimes B)$-module structure of $M \outertensor N$ is given by
%   \[
%     A \otimes B
