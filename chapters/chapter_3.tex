...
Another ``famous'' family of polynomial functions in $\mc{P}(\Mat_n(k))^{\GL_n(k)}$ (where $\GL_n(k)$ acts on $\Mat_n(k)$ by conjugation) are the so called power traces.
...
\begin{proof}
  ...
  Then $1 \otimes w_1$, \dots, $1 \otimes w_n$ is an $L$-basis of $W_L$.
...
\begin{prop}
  ...
  \[
    ...
    \to     \mc{P}_L(W_L),
            \lambda \otimes f
    \emapsto \lambda \iota(f)
  \]
  is an isomorphism of $L$-algebras, where $\iota \colon \mc{P}_k(W) \to \mc{P}_L(W_L)$ is the inclusion as before.
\end{prop}
\begin{proof}
  Let $v_1$, \dots, $v_n$ be a $k$-basis of $W$ and $\varphi_1, \dotsc, \varphi_n \in \mc{P}_k(W)$ the corresponding coordinate functions.
...
Given a $k$-vector space $W$ and a $k$-basis $w_1$, \dots, $w_n$ of $W$ we have an isomorphism
\[
        k[X_1, \dotsc, X_n]
  \cong \mc{P}_k(W)
\]
