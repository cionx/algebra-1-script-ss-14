...
\begin{proof}
  ...
  For every homomorphism of $k$-algebras $F \colon \mc{P}(W) \to \mc{P}(V)$ we define $F^\circ_j \defined F(\psi_j)$ for every $j = 1, \dotsc, m$.
  Then
  \[
            F^\circ
    \colon  V
  ...
  Let $f \colon V \to W$ be a polynomial map and let $f_1, \dotsc, f_m \in \mc{P}(V)$ be the polynomial functions $f_j \colon V \to k$ with
  \[
      f(v)
    = f_1(v) w_1 + \dotsb + f_m(v) w_m
  \]
  for every $v \in V$.
  For $F \defined f^*$ we then have that $F^\circ_j = F(\psi_j) = f^*(\psi) = \psi_j \circ f = f_j$ for every $j = 1, \dotsc, m$ and therefore $(f^*)^\circ = F^\circ = f$.
  ...
  The coordinate ring $\mc{P}(V)$ is generated by the coordinate functions $\psi_1, \dotsc, \psi_m$ as a $k$-algebra, so it follows that $(F^\circ)^* = f^* = F$.
