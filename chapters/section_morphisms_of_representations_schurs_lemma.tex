...
\begin{example}
  ...
  \begin{enumerate}
    \item
      It follows for $\ringchar(k) = 0$ that $\Integer/n$ has precisely $n$ pairwise non-isomorphic irreducible representations over $k$.
    \item
      If $\ringchar(k) = p$ and $n = p^r n'$ with $p \ndivides n'$ then
      \[
          X^n - 1
        = X^{p^r n'} - 1^{p^r}
        = (X^{n'} - 1)^{p^r}
