...
Given $k$-algebras $A_1$ and $A_2$ and a homomorphism of $k$-algebras
\[
 f \colon A_1 \to A_2
\]
...
\]
is a homomorphism of $L$-algebras. It is multiplicative on the simple tensors since for all simple tensors $\lambda_1 \otimes a_1, \lambda_2 \otimes a_2 \in L \otimes_k A_1$
\begin{align*}
...
Given a $k$-algebra $A$, an $L$-algebra $B$ and a homomorphism of $k$-algebras $f \colon A \to B$ the corresponding $L$-linear map $\hat{f} \colon A_L \to B$ is a homomorphism of $L$-algebras: Notice that for all simple tensors $\lambda_1 \otimes a_1, \lambda_2 \otimes a_2 \in A_L$ we have
\begin{align*}
...
In the special case of the $k$-algebra $k[X_1, \dotsc, X_n]$ this gives us an $L$-algebra
\[
...
\end{prop}
\begin{proof}
 Too see that $\Phi$ is well-defined and $L$-linear notice that it is the $L$-linear map which corresponds to the $k$-linear inclusion
 \[
  k[X_1, \dotsc, X_n] \hookrightarrow L[X_1, \dotsc, X_n],
 \]
 i.e.\ it is the unique $L$-linear map making the diagram
 \begin{center}
  \tikzsetnextfilename{inclusion_isomorphism_polynomial_rings}
  \begin{tikzpicture}[node distance = 6em, auto]
   ...
   \draw[->] (kXL) to node[below] {$\Phi$} (LX);
  \end{tikzpicture}
 \end{center}
 commute. Since the inclusion $k[X_1, \dotsc, X_n] \hookrightarrow L[X_1, \dotsc, X_n]$ is a homomorphism of $k$-algebras $\Phi$ is an homomorphism of $L$-algebras.

 Using the universal property of the polynomial ring we find that there exists an unique homomorphism of $L$-algebras
 \[
  \Psi \colon L[X_1, \dotsc, X_n] \to k[X_1, \dotsc, X_n]_L, X_i \mapsto 1 \otimes X_i,
 \]
 
 Notice that for all $1 \leq i \leq n$ we have
 \[
  (\Phi\Psi)(X_i)
  = \Phi(1 \otimes X_i)
  = X_i,
 \]
 so $\Phi \Psi = \id_{L[X_1, \dotsc, X_n]}$ by the universal property of the polynomial ring. We also find that for all $\lambda \in L$ and $f \in k[X_1, \dotsc, X_n]$
 \begin{align*}
  ...
  = \lambda \otimes f,
 \end{align*}
 and since the simple tensors $\lambda \otimes f$ generate $k[X_1, \dotsc, X_n]_L$ as a $k$-vector space we get $\Phi \Psi = \id_{k[X_1, \dotsc, X_n]_L}$. This shows that $\Phi$ and $\Psi$ are isomorphisms of $k$-algebras with $\Phi^{-1} = \Psi$.
\end{proof}
...
\end{prop}
\begin{proof}
 Similar to the previous proof we we notice that the canonical inclusion $\Mat_n(k) \hookrightarrow \Mat_n(L)$ induces $\Phi$ as an homomorphism of $L$-algebras. Since this inclusion restricts to a bijection between the $k$-basis $(E_{ij})_{1 \leq i,j \leq n}$ of $\Mat_n(k)$ and the $L$-basis $(E_{ij})_{1 \leq i,j \leq n}$ of $\Mat_n(L)$ we find that $\Phi$ restricts to a bijection between the $L$-basis $(1 \otimes E_{ij})_{1 \leq i,j \leq n}$ of $L \otimes_k \Mat_n(k)$ and the $L$-basis $(E_{ij})_{1 \leq i,j \leq n}$ of $\Mat_n(L)$. This shows $\Phi$ is an isomorphim of $L$-vector spaces and therefore one of $L$-algebras.
\end{proof}
