...
\]
If $A_1$ and $A_2$ are unitial, then so is $A_1 \otimes_k A_2$. To see that this multiplication is well-defined notice that the map
\begin{align*}
...
\begin{align*}
 ...
 (a_1 \otimes b_1, a_2 \otimes b_2) &\mapsto (a_1 a_2) \otimes (b_1 b_2).
\end{align*}
is also $L$-bilinear. If $A$ is unital then so is $A_L$.
...
\begin{lem}
 Let $A$ be an unitial $k$-algebra and $I \subseteq A$ a left-sided ideal generated by elements $(b_j)_{j \in J}$. Then the left-sided ideal $I_L \subseteq A_L$ is generated by $(1 \otimes b_j)_{j \in J}$.
\end{lem}
\begin{proof}
 Let $I_0$ be the left-sided ideal in $A_L$ generated by the elements $(1 \otimes b_j)_{j \in J}$. Since $I_L$ is an left-ideal with $1 \otimes b_j \in I_L$ for all $j \in J$ we have $I_0 \subseteq I_L$. On the other hand we have
 \begin{align*}
  I_0
  &= \sum_{j \in J} A_L (1 \otimes b_j)
  = \sum_{j \in J} \left( \sum_{\substack{\lambda \in L \\ a \in A}} k (\lambda \otimes a) \right) (1 \otimes b_j) \\
  &= \sum_{\substack{j \in J \\ \lambda \in L \\ a \in A}} k (\lambda \otimes (a b_j))
  = \sum_{\substack{j \in J \\ a \in A}} L \underbrace{(1 \otimes (a b_j))}_{\in I_L}
  \subseteq I_L.
 \end{align*}
 Since $A_L$ is generated by the simple tensors $\lambda \otimes a$ as a $k$-vector space.
\end{proof}


\begin{rem}
 Similar statements can be made about right-sided ideals and two-sided ideals.
\end{rem}
