...
It is important to notice that for all $k$-vector spaces $V$ and $V'$ and every $k$-linear map $f : V \to V'$ the diagram
...
\[
 \Phi_{V,W} : \Hom_L(E(V),W) \cong \Hom_k(V,R(W))
\]
...
\end{prop}
\begin{proof}
 We need to check that for all $k$-vector spaces $V$ and $V'$, all $L$-vector spaces $W$ and $W'$, every $k$-linear map $f : V' \to V$ (notice that $f$ goes from $V'$ to $V$) and every $L$-linear map $g : W \to W'$ the diagram
...
\begin{prop}
 ...
 \[
  \Phi: L \otimes_k k[X_1, \ldots, X_n] \to L[X_1, \ldots, X_n], \lambda \otimes f \mapsto \lambda f.
 \]
\end{prop}
\begin{proof}
