...
\begin{defi}
 Given a set $X$ let
 \[
  S(X) := \{f \colon X \to X \mid f \text{ is bijective}\}.
 \]
...
\begin{defi}
 ...
 \[
  X^G := \{\text{$G$-invariants of $X$}\} = \{x \in X \mid g.x = x \text{ for all } g \in G\}.
 \]
...
\begin{proof}
 \begin{enumerate}[a)]
  ...
  \item
   Let $f \in \Maps(X,k)^G$ and $\mc{O}_1, \dotsc, \mc{O}_n$ be the orbits in $X$. Pick a representative $x_i \in \mc{O}_i$ for all $1 \leq i \leq n$. Then $f = \sum_{i=1}^n f(x_i) \chi_{\mc{O}_i}$. This adds because
   \[
...
\[
 \GL(V) := \{f \colon V \to V \mid f \text{ is $k$-linear and invertible}\}.
\]
...
\begin{expls}
 ...
 If $v_1, \dotsc, v_n$ is a basis of $V$ and $w_1, \dotsc, w_m$ a basis of $W$ we can write the action of $g \in G$ on $V$, resp. $W$, as a matrix $A$, resp. $B$. The actions $(1)$ and $(2)$ are then given by matrices
 \[
  \begin{pmatrix}
   A & 0 \\
   0 & B
  ...
  \end{pmatrix}
\]
with respect to the basis $v_1, \dotsc, v_n, w_1, \dotsc, w_m$ of $V \oplus W$ and $v_1 \otimes w_1, v_1 \otimes w_2, \dotsc, v_n \otimes w_m$ of $V \otimes W$.
\end{expls}
...
\begin{rem}
 ...
 \[
  ...
  := \Hom_G(V \oplus \dotsb \oplus V, V \oplus \dotsb \oplus V)
  \cong \Mat(n \times n, k)
 \]
 is an isomorphism of rings by Schur’s lemma part c).
 
 More general: Let $V_1, \dotsc, V_r$ be pairwise non-isomorphic irreducible finite dimensioal representations of some group $G$ and $W_i := V_i^{n_i}$. Then
 \begin{align*}
...
\begin{expl}
 ...
 \[
  g.e_i = e_{g(i)}
 \]
 where $e_1, \dotsc, e_n$ denotes the standard basis of $k^n$. Extending this linearly we get a representation of $G$ on $k^n$, i.e.
 \[
...
\begin{lem}
 Let $\{e_1, \dotsc, e_n\}$ denote the standard basis von $k^n$. $G = S_n$ acts linearly on $k^n$ as in the previous example.
 \begin{enumerate}[a)]
  \item
  ...
  \shortintertext{and}
   U_2 := \left\{ (\lambda_1, \dotsc, \lambda_n) \in k^n \,\middle|\, \sum_{i=1}^n \lambda_i = 0 \right\}
  \end{gather*}
